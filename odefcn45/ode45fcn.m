...
function xd=fdy2(t,x,tf,ts,qd,dqd,ddqd)            %% ~ =t is Independent variable must be exist
    ...
    rho1=200; rho2=100; alpha1=0.6; alpha2=1.65; mu=0.15; L1=((alpha2-alpha1)/(alpha2-1))*mu^(alpha1-1);
    L2=((alpha1-1)/(alpha2-1))*mu^(alpha1-alpha2); gama1=200; gama2=20;  rata=0.05; fai=0.05;
    ...
    tau=M_q*(ddq_d.' + rho1*de   + rho2*d_theta + gama1*s + (deta + gama2)*Sat_s ) + Cor * dq + G_q  ;
